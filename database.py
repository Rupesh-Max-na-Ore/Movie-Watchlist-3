...
INSERT_WATCHED_MOVIE = (
    "INSERT INTO watched (user_username, movie_id, review, rating) VALUES (, , , %s);"
)
SET_MOVIE_WATCHED = "UPDATE movies SET watched = 1 WHERE title = %s;"
SEARCH_MOVIES = "SELECT * FROM movies WHERE title LIKE %s COLLATE NOCASE;"
...
def create_tables():
    with connection:
        with connection.cursor() as cursor:
            ...
            cursor.execute(CREATE_PLANNED_TABLE)
...
def watch_movie(username, movie_id, review=None, rating=None):
    with connection:
        with connection.cursor() as cursor:
            ...
            connection.execute(
                "INSERT OR IGNORE INTO users (username) VALUES (%s);", (username,)
            )
            # Check if already watched
            cursor = connection.cursor()
            cursor.execute(
                "SELECT 1 FROM watched WHERE user_username = %s AND movie_id = %s;",
                (username, movie_id),
            )
            if cursor.fetchone():
                print(
                ...
                return
            connection.execute(
                INSERT_WATCHED_MOVIE, (username, movie_id, review, rating)
            )
    ...
    with connection:
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                FROM watched
                WHERE movie_id = %s AND (review IS NOT NULL OR rating IS NOT NULL)
                """,
                (movie_id,),
            )
            return cursor.fetchall()
